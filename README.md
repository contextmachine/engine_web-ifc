...
3. **Native Python bindings.** [`python/`](python/) contains `webifc`, a Cython package over the C++ core (no WASM): numpy float64 geometry, `GetLine`-compatible attribute access, streaming from disk, and a `webifc.bim` module for full BIM data extraction (spatial hierarchy, property/quantity sets, materials, types, connections, systems). See [python/README.md](python/README.md). Its pytest suite is the fork's primary regression harness (exact mesh-count parity with `benchmark.md` plus pinned geometry digests):
